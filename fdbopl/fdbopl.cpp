...
/* Faster DBopl modified by hobum lee */
...
namespace FDBOPL {
    ...
    // noise shifts. originally WAVE_SH - 10. subtracter is the total bits of "count" in ForwardNoise()
    // lower subtracter lowers cpu power and also lowers the noise quality.
    #define NOISE_SH ( WAVE_SH - 2 )

    //Maximum amount of attenuation bits
    //Envelope goes to 511, 9 bits
    #if (FDBOPL_WAVE == FWAVE_TABLEMUL )
    ...
    #if ( FDBOPL_WAVE == FWAVE_HANDLER ) || ( FDBOPL_WAVE == FWAVE_TABLELOG )
    static Bit16u ExpTable[ 256 ];
    #endif

    #if ( FDBOPL_WAVE == FWAVE_HANDLER )
    ...
    */
    ...
    #endif // #if ( FDBOPL_WAVE == FWAVE_HANDLER )
    ...
    */
    ...
    #elif ( FDBOPL_WAVE == FWAVE_TABLEMUL )
        return (waveBase[ index & waveMask ] * MulTable[ vol >> ENV_EXTRA ]) >> MUL_SH;
    #elif ( FDBOPL_WAVE == FWAVE_TABLELOG )
    ...
    */
    ...
    Channel::Channel() {
        ...
        feedback = 31;
        fourMask = 0;
        synthHandler = &Channel::BlockTemplate_2FM;//< sm2FM >;
    };
    ...
    void Channel::WriteC0( const Chip* chip, Bit8u val ) {
        ...
        if ( chip->opl3Active ) {
            ...
            if ( (chip->reg104 & fourMask) & 0x3f ) {
                ...
                {
                    case 0:
                        chan0->synthHandler = &Channel::BlockTemplate_3FMFM;//< sm3FMFM >;
                        break;
                    case 1:
                        chan0->synthHandler = &Channel::BlockTemplate_3AMFM;//< sm3AMFM >;
                        break;
                    case 2:
                        chan0->synthHandler = &Channel::BlockTemplate_3FMAM;//< sm3FMAM >;
                        break;
                    case 3:
                        chan0->synthHandler = &Channel::BlockTemplate_3AMAM;//< sm3AMAM >;
                        break;
                }
            ...
            //Regular dual op, am or fm
                ...
                synthHandler = &Channel::BlockTemplate_3AM;//< sm3AM >;
            } else {
                synthHandler = &Channel::BlockTemplate_3FM;//< sm3FM >;
            }
        ...
        //opl2 active
            ...
            //Regular dual op, am or fm
                ...
                synthHandler = &Channel::BlockTemplate_2AM;//< sm2AM >;
            } else {
                synthHandler = &Channel::BlockTemplate_2FM;//< sm2FM >;
            }
        }
...
// caching Op()s to vars
#define INIT_VAR_OP01() Operator* op0 = Op(0); Operator* op1 = Op(1)
#define INIT_VAR_OP23() Operator* op2 = Op(2); Operator* op3 = Op(3)
#define INIT_VAR_OP45() Operator* op4 = Op(4); Operator* op5 = Op(5)

//Init the operators with the the current vibrato and tremolo values
#define PREPARE_OP01( chip ) op0->Prepare( chip ); op1->Prepare( chip )
#define PREPARE_OP23( chip ) op2->Prepare( chip ); op3->Prepare( chip )
#define PREPARE_OP45( chip ) op4->Prepare( chip ); op5->Prepare( chip )

#define RETURN_2OP() return (this + 1)
#define RETURN_4OP() return (this + 2)
#define RETURN_6OP() return (this + 3)

#define CLEAR_HISTORY() old[0] = old[1] = 0

#define IF_AM_SILENT_THEN_RETURN() \
    if( op0->Silent() && op1->Silent() ) {\
        CLEAR_HISTORY();\
        RETURN_2OP();\
    }

#define IF_FM_SILENT_THEN_RETURN() \
    if ( op1->Silent() ) {\
        CLEAR_HISTORY();\
        RETURN_2OP();\
    }

#define IF_FMFM_SILENT_THEN_RETURN() \
    if ( op3->Silent() ) {\
        CLEAR_HISTORY();\
        RETURN_4OP();\
    }

#define IF_AMFM_SILENT_THEN_RETURN() \
    if ( op0->Silent() && op3->Silent() ) {\
        CLEAR_HISTORY();\
        RETURN_4OP();\
    }

#define IF_FMAM_SILENT_THEN_RETURN() \
    if ( op1->Silent() && op3->Silent() ) {\
        CLEAR_HISTORY();\
        RETURN_4OP();\
    }

#define IF_AMAM_SILENT_THEN_RETURN() \
    if ( op0->Silent() && op2->Silent() && op3->Silent() ) {\
        CLEAR_HISTORY();\
        RETURN_4OP();\
    }
...
#define PREPARE_OUTPUT() \
    Bit32s mod = (Bit32u)((old[0] + old[1])) >> feedback;\
    old[0] = old[1];\
    old[1] = (Bit32s)op0->GetSample( mod )


#define GET_AM_SAMPLE() Bit32s sample = (Bit32s)(old[0] + op1->GetSample( 0 ))

#define GET_FM_SAMPLE() Bit32s sample = (Bit32s)op1->GetSample( old[0] )

#define GET_FMFM_SAMPLE()\
    Bits next = op1->GetSample( old[0] );\
    next = op2->GetSample( next );\
    Bit32s sample = (Bit32s)op3->GetSample( next )

#define GET_AMFM_SAMPLE()\
    Bit32s sample = old[0];\
    Bits next = op1->GetSample( 0 );\
    next = op2->GetSample( next );\
    sample += (Bit32s)op3->GetSample( next )

#define GET_FMAM_SAMPLE()\
    Bit32s sample = (Bit32s)op1->GetSample( old[0] );\
    Bits next = op2->GetSample( 0 );\
    sample += (Bit32s)op3->GetSample( next )

#define GET_AMAM_SAMPLE()\
    Bit32s sample = old[0];\
    Bits next = op1->GetSample( 0 );\
    sample += (Bit32s)op2->GetSample( next );\
    sample += (Bit32s)op3->GetSample( 0 )


#define OUT_MONO( i ) output[i] += sample
...
#define OUT_MONO_PERC( i ) \
    sample <<= 1;\
    output[i] += sample

#define OUT_STEREO_PERC( i ) \
    sample <<= 1;\
    output[i * 2 + 0] += sample;\
    output[i * 2 + 1] += sample
...
#define PREPARE_BD() \
    ...
    Bit32s sample = (Bit32s)op1->GetSample( mod )

    //Precalculate stuff used by other outputs
#define PREPARE_NOISE_PHASE_BIT() \
    Bit32u noiseBit = chip->ForwardNoise() & 0x1;\
    Bit32u c2 = (Bit32u)op2->ForwardWave();\
    Bit32u c5 = (Bit32u)op5->ForwardWave();\
    Bit32u phaseBit = (((c2 & 0x88) ^ ((c2<<5) & 0x80)) | ((c5 ^ (c5<<2)) & 0x20)) ? 0x02 : 0x00

    //Hi-Hat
#define PREPARE_HIHAT() \
    Bit32u hhVol = (Bit32u)op2->ForwardVolume();\
    if ( !ENV_SILENT( hhVol ) ) {\
        Bit32u hhIndex = (phaseBit<<8) | (0x34 << ( phaseBit ^ (noiseBit << 1 )));\
        sample += (Bit32s)op2->GetWave( hhIndex, hhVol );\
    }

    //Snare Drum
#define PREPARE_SNARE() \
    Bit32u sdVol = (Bit32u)op3->ForwardVolume();\
    if ( !ENV_SILENT( sdVol ) ) {\
        Bit32u sdIndex = ( 0x100 + (c2 & 0x100) ) ^ ( noiseBit << 8 );\
        sample += (Bit32s)op3->GetWave( sdIndex, sdVol );\
    }

    //Tom-tom
#define PREPARE_TOMTOM() \
    sample += (Bit32s)op4->GetSample( 0 )

    //Top-Cymbal
#define PREPARE_CYMBAL() \
    Bit32u tcVol = (Bit32u)op5->ForwardVolume();\
    if ( !ENV_SILENT( tcVol ) ) {\
        Bit32u tcIndex = (1 + phaseBit) << 8;\
        sample += (Bit32s)op5->GetWave( tcIndex, tcVol );\
    }


    Channel* Channel::BlockTemplate_2AM( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();	
        IF_AM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_3AM( Chip* chip, Bit32u samples, Bit32s* output ) 
    {
        INIT_VAR_OP01();	
        IF_AM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_2FM( Chip* chip, Bit32u samples, Bit32s* output ) 
    {
        INIT_VAR_OP01();	
        IF_FM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_3FM( Chip* chip, Bit32u samples, Bit32s* output ) 
    {
        INIT_VAR_OP01();
        IF_FM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_3FMFM( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();
        INIT_VAR_OP23();
        IF_FMFM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_3AMFM( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();
        INIT_VAR_OP23();
        IF_AMFM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_3FMAM( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();
        INIT_VAR_OP23();
        IF_FMAM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_3AMAM( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();
        INIT_VAR_OP23();
        IF_AMAM_SILENT_THEN_RETURN();
    ...
    Channel* Channel::BlockTemplate_2PERC( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();
        INIT_VAR_OP23();
        INIT_VAR_OP45();
    ...
    Channel* Channel::BlockTemplate_3PERC( Chip* chip, Bit32u samples, Bit32s* output )
    {
        INIT_VAR_OP01();
        INIT_VAR_OP23();
        INIT_VAR_OP45();
...
//        Bitu count = noiseCounter >> LFO_SH;
        Bitu count = noiseCounter >> NOISE_SH;
    ...
    void Chip::WriteBD( Bit8u val ) {
        ...
        if ( val & 0x20 ) {
            ...
            if ( change & 0x20 ) {
                if ( opl3Active ) {
                    chan[6].synthHandler = &Channel::BlockTemplate_3PERC;//< sm3Percussion >;
                } else {
                    chan[6].synthHandler = &Channel::BlockTemplate_2PERC;//< sm2Percussion >;
                }
            }
            //Bass Drum
            if ( val & 0x10 ) {
...
#define REGCHAN( _FUNC_ )																\
    ...
    void Chip::GenerateBlock2( Bitu total, Bit32s* output ) {
        while ( total > 0 ) {
            Bit32u samples = (Bit32u)ForwardLFO( (Bit32u)total );
...
//        noiseAdd = (Bit32u)( 0.5 + scale * ( 1 << LFO_SH ) );
        noiseAdd = (Bit32u)( 0.5 + scale * ( 1 << NOISE_SH ) );
        noiseCounter = 0;
    ...
    #if (( FDBOPL_WAVE == FWAVE_TABLELOG ) || ( FDBOPL_WAVE == FWAVE_TABLEMUL ))
        for ( int i = 0; i < 256; i++ ) {
            //Fill silence gaps
...
static void ConvertBuffer32to16( const Bit32s* const buf32, short* buf16, int count )
{
	for( int i = 0; i < count; ++i )
	{
		buf16[i] = (short)buf32[i];
	}
}

CFDBopl::CFDBopl( int rate, bool stereo ) : _rate( rate )
{
	currType = TYPE_OPL3;
	_stereo = stereo;
	init();
...
void CFDBopl::update(short *buf, int samples) // fill buffer
{
	Bit32s buffer32[1024 * 2];

	if( _stereo )
	{
		_chip.GenerateBlock3( samples, buffer32 );
		ConvertBuffer32to16( buffer32, buf, samples * 2 );
	}
	else
	{
		_chip.GenerateBlock2( samples, buffer32 );
		ConvertBuffer32to16( buffer32, buf, samples );
	}
}

void CFDBopl::write(int reg, int val)
{
	Bit32u port = (currChip > 0) ? 0x38a : 0x388;
	Bit32u resReg = _chip.WriteAddr( port, (Bit8u)reg );

	_chip.WriteReg( resReg, (Bit8u)val );
}
